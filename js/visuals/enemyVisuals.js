...
export function drawEnemyVisual(ctx, game, e) {
  ...
  switch (e.type) {
    ...
    case 'demon': {
      ctx.rotate(facing*.18);ctx.fillStyle=hurt?'#fff':'#8d313b';ctx.beginPath();ctx.ellipse(0,3,r*.62,r*.82,0,0,Math.PI*2);ctx.fill();ctx.fillStyle='#b94a53';ctx.beginPath();ctx.arc(0,-r*.55,r*.4,0,Math.PI*2);ctx.fill();ctx.fillStyle='#e4c0a6';poly(ctx,[[-r*.28,-r*.78],[-r*.66,-r*1.15],[-r*.12,-r*.88]]);ctx.fill();poly(ctx,[[r*.28,-r*.78],[r*.66,-r*1.15],[r*.12,-r*.88]]);ctx.fill();ctx.strokeStyle='#7c2630';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(-r*.35,r*.15);ctx.lineTo(-r*.72,r*.62);ctx.moveTo(r*.35,r*.15);ctx.lineTo(r*.72,r*.62);ctx.stroke();drawEyes(ctx,-r*.14,r*.14,-r*.56,'#ffd86d');break;
    }
    case 'specter': {
      glow(ctx,q,'#9a8dea',12);ctx.globalAlpha*=.78;ctx.fillStyle=hurt?'#fff':'#6b5bb2';ctx.beginPath();ctx.moveTo(0,-r*.9);ctx.bezierCurveTo(r*.7,-r*.5,r*.55,r*.5,r*.28,r*.9);ctx.quadraticCurveTo(0,r*.55,-r*.28,r*.9);ctx.bezierCurveTo(-r*.55,r*.5,-r*.7,-r*.5,0,-r*.9);ctx.fill();ctx.fillStyle='#dcd8ff';drawEyes(ctx,-r*.13,r*.13,-r*.38,'#e9e7ff');if(q>=1){ctx.strokeStyle='#a89cff';ctx.lineWidth=1.5;for(let i=0;i<3;i++){const a=t*1.1+i*Math.PI*2/3;ctx.beginPath();ctx.arc(Math.cos(a)*r*.55,Math.sin(a)*r*.32+6,3,0,Math.PI*2);ctx.stroke();}}break;
    }
    case 'ogreBoss': {
    ...
    case 'graveTyrant': {
      glow(ctx,q,'#b9a982',14);ctx.fillStyle=hurt?'#fff':'#4a4438';ctx.fillRect(-r*.48,-r*.45,r*.96,r*1.25);
      ctx.fillStyle='#d8cfb5';ctx.beginPath();ctx.arc(0,-r*.55,r*.34,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#d6b867';ctx.lineWidth=3;
      for(let i=-1;i<=1;i++){ctx.beginPath();ctx.moveTo(i*r*.18,-r*.77);ctx.lineTo(i*r*.24,-r*1.12);ctx.stroke();}
      ctx.strokeStyle='#88745c';ctx.lineWidth=7;ctx.beginPath();ctx.moveTo(r*.42,-r*.15);ctx.lineTo(r*1.05,r*.82);ctx.stroke();break;
    }
    case 'arcaneBehemoth': {
      glow(ctx,q,'#cb6cff',18);ctx.fillStyle=hurt?'#fff':'#47265a';poly(ctx,[[-r*.78,-r*.55],[-r*.95,r*.38],[-r*.4,r],[r*.4,r],[r*.95,r*.38],[r*.78,-r*.55],[0,-r*.92]]);ctx.fill();
      ctx.fillStyle='#e56fff';ctx.beginPath();ctx.arc(0,0,r*.18,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#d99cff';ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,0,r*.48,t*.5,t*.5+Math.PI*1.6);ctx.stroke();break;
    }
    case 'plagueMother': {
      glow(ctx,q,'#98d85f',12);ctx.fillStyle=hurt?'#fff':'#4e7133';ctx.beginPath();ctx.ellipse(0,5,r*.78,r*.92,0,0,Math.PI*2);ctx.fill();
      ctx.fillStyle='#94c66b';for(let i=0;i<6;i++){const a=i*Math.PI/3;ctx.beginPath();ctx.arc(Math.cos(a)*r*.62,Math.sin(a)*r*.52,5,0,Math.PI*2);ctx.fill();}
      drawEyes(ctx,-r*.18,r*.18,-r*.28,'#eaff8a');break;
    }
    case 'frostColossus': {
      glow(ctx,q,'#9ceeff',15);ctx.fillStyle=hurt?'#fff':'#5c8c98';poly(ctx,[[-r*.7,-r*.66],[-r*.92,r*.25],[-r*.5,r],[r*.5,r],[r*.92,r*.25],[r*.7,-r*.66],[0,-r]]);ctx.fill();
      ctx.fillStyle='#bdf5ff';for(let i=-1;i<=1;i++){poly(ctx,[[i*r*.28,-r*.62],[i*r*.18-r*.12,-r*1.15],[i*r*.18+r*.12,-r*.8]]);ctx.fill();}break;
    }
    case 'infernalWyrm': {
      ctx.rotate(facing);glow(ctx,q,'#ff6a38',16);ctx.fillStyle=hurt?'#fff':'#9b382b';ctx.beginPath();ctx.ellipse(-r*.08,0,r*.95,r*.42,0,0,Math.PI*2);ctx.fill();
      ctx.beginPath();ctx.arc(r*.7,-r*.08,r*.36,0,Math.PI*2);ctx.fill();ctx.fillStyle='#f0b08a';poly(ctx,[[r*.8,-r*.3],[r*1.15,-r*.7],[r*.95,-r*.16]]);ctx.fill();break;
    }
    case 'stormHerald': {
      glow(ctx,q,'#ffe45b',17);ctx.fillStyle=hurt?'#fff':'#544e33';poly(ctx,[[-r*.58,r],[r*.58,r],[r*.4,-r*.25],[0,-r*.95],[-r*.4,-r*.25]]);ctx.fill();
      ctx.strokeStyle='#ffe45b';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(-r*.8,-r*.2);ctx.lineTo(-r*.35,-r*.55);ctx.lineTo(-r*.05,-r*.1);ctx.moveTo(r*.1,r*.2);ctx.lineTo(r*.45,-r*.55);ctx.lineTo(r*.82,-r*.12);ctx.stroke();break;
    }
    case 'titanRoots': {
      ctx.fillStyle=hurt?'#fff':'#4e7135';ctx.beginPath();ctx.ellipse(0,2,r*.72,r*.88,0,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#75623c';ctx.lineWidth=7;
      for(let i=-2;i<=2;i++){ctx.beginPath();ctx.moveTo(i*r*.16,-r*.45);ctx.lineTo(i*r*.28,-r*1.02);ctx.stroke();}ctx.strokeStyle='#6d8d4a';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(-r*.55,r*.15);ctx.lineTo(-r*.95,r*.78);ctx.moveTo(r*.55,r*.15);ctx.lineTo(r*.95,r*.78);ctx.stroke();break;
    }
    case 'voidReaper': {
      glow(ctx,q,'#9f6de0',16);ctx.fillStyle=hurt?'#fff':'#2d1d43';poly(ctx,[[-r*.64,r],[r*.64,r],[r*.38,-r*.15],[0,-r*.92],[-r*.38,-r*.15]]);ctx.fill();
      ctx.strokeStyle='#c49aff';ctx.lineWidth=6;ctx.beginPath();ctx.arc(r*.48,-r*.1,r*.72,-1.2,1.15);ctx.stroke();ctx.strokeStyle='#6a467f';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(r*.45,-r*.55);ctx.lineTo(r*.85,r*.82);ctx.stroke();break;
    }
    case 'finalBoss': {
...
export function drawEnemyDeathVisual(ctx, game, d) {
  ...
  } else if (d.type === 'specter') {
    ctx.strokeStyle='#9e91ee';ctx.lineWidth=2;for(let i=0;i<(q===0?3:6);i++){const a=i*Math.PI*2/6;ctx.beginPath();ctx.arc(Math.cos(a)*r*(1-life),Math.sin(a)*r*(1-life),r*.35*life,0,Math.PI*2);ctx.stroke();}
  } else if (d.type === 'demon') {
    ctx.fillStyle='#7e2634';for(let i=0;i<6;i++){const a=i*Math.PI*2/6;poly(ctx,[[Math.cos(a)*r*(1-life),Math.sin(a)*r*(1-life)],[Math.cos(a+.2)*r*(1.3-life*.2),Math.sin(a+.2)*r*(1.3-life*.2)],[Math.cos(a-.2)*r*(1.1-life*.1),Math.sin(a-.2)*r*(1.1-life*.1)]]);ctx.fill();}
  } else if (d.type === 'slime') {
    ctx.fillStyle='#67c86d';for(let i=0;i<(q===0?5:9);i++){const a=i*Math.PI*2/9;ctx.beginPath();ctx.ellipse(Math.cos(a)*r*(1-life)*1.4,Math.sin(a)*r*(1-life),r*.14*life,r*.09*life,a,0,Math.PI*2);ctx.fill();}
