...
export class Game {
  ...
  applyEliteAura(enemy) {
    if (enemy.eliteAura === 'fire') {
      ...
      this.player.speed = Math.max(this.player.baseSpeed * .72, this.player.speed * .999);
  ...
  updateMeteors(dt) {
    ...
    for (const m of this.meteors) {
      ...
      if (m.t <= 0 && !m.hit) {
        ...
        this.burst(m.x, m.y, this.colorFor(m.element), 26);
        this.shake = .13;
      }
      ...
      if (target) {
        ...
        if (d < 30 + target.size && s.cd <= 0) {
          ...
          if (s.electric) {
            ...
            for (const e of chain) this.damageEnemy(e, s.damage * .45, false, 'electric', true, s.id);
          }
  ...
  burst(x, y, color, n) {
    const quality = Number(this.save.data.settings.particles) || 0;
    if (quality <= 0) return;
    const count = Math.min(44, Math.round(n * Math.max(.5, quality * .75)));
    for (let i = 0; i < count && this.particles.length < GAME_CONFIG.maxParticles; i++) {
      const a = Math.random() * Math.PI * 2;
      const speed = 45 + Math.random() * 130;
      this.particles.push(this.particlePool.acquire({ x, y, vx: Math.cos(a) * speed, vy: Math.sin(a) * speed, life: .25 + Math.random() * .45, color, r: 2 + Math.random() * 2 }));
    }
  }
  ...
  render() {
    ...
    for (const o of this.orbitVisuals) this.drawCircleWorld(c, o.x, o.y, o.r, o.color || '#d4d8cc');
    this.orbitVisuals = [];
    for (const a of this.lightningArcs) this.drawLightning(c, a);
    for (const p of this.particles) this.drawCircleWorld(c, p.x, p.y, p.r || 3, p.color);
    this.drawPlayer(c);
  ...
  drawProjectile(c, p) { this.drawCircleWorld(c, p.x, p.y, Math.min(p.r, 20), this.colorFor(p.element)); }
  ...
  drawSummon(c, s) {
    this.drawCircleWorld(c, s.x, s.y, s.id === 'revenant' ? 8 : 10, s.electric ? '#ffe45b' : '#c9c2a2');
  }

  drawMeteor(c, m) {
    if (!this.camera.visible(m.x, m.y, m.r)) return;
    const s = this.camera.worldToScreen(m.x, m.y);
    c.strokeStyle = this.colorFor(m.element);
    c.lineWidth = 3;
    c.beginPath();
    c.arc(s.x, s.y, m.r * Math.max(.25, 1 - m.t * .3), 0, Math.PI * 2);
    c.stroke();
  }
  ...
  drawLightning(c, a) {
    const p1 = this.camera.worldToScreen(a.x1, a.y1);
    const p2 = this.camera.worldToScreen(a.x2, a.y2);
    c.save();
    c.globalAlpha = Math.min(1, a.life / .12);
    c.strokeStyle = a.color || '#ffe45b';
    c.lineWidth = 2;
    c.beginPath();
    c.moveTo(p1.x, p1.y);
    c.lineTo(p2.x, p2.y);
