import {
  ...
  timeHpMultiplier
} from './config.js?v=2.7.0';
import { Player } from './entities/player.js?v=2.7.0';
import { Enemy } from './entities/enemy.js?v=2.7.0';
import { Camera } from './camera.js?v=2.7.0';
import { SpatialGrid } from './systems/spatialGrid.js?v=2.7.0';
import { ObjectPool } from './systems/objectPool.js?v=2.7.0';
import { WeaponSystem, WEAPONS } from './weapons/weapons.js?v=2.7.0';
import { getWeaponLevelDescription } from './weapons/weaponData.js?v=2.7.0';
import { availableTransformations, replaceAbility, autoEvolutionFor, evolutionForBase, ARCANE_FUSIONS } from './evolutions.js?v=2.7.0';
import { FUSION_ITEMS } from './items/fusionItems.js?v=2.7.0';
import { RELICS, rollRelic } from './relics.js?v=2.7.0';
import { checkAchievements } from './achievements.js?v=2.7.0';
import { ClassPassiveSystem, drawFallenAllyVisual } from './systems/classPassives.js?v=2.7.0';
import { WorldMap } from './world/worldMap.js?v=2.7.0';
import { MinimapSystem } from './systems/minimap.js?v=2.7.0';
import { drawPlayerVisual } from './visuals/playerVisuals.js?v=2.7.0';
import {
...
} from './visuals/abilityVisuals.js?v=2.7.0';
import {
  drawEnemyVisual, drawEnemyProjectileVisual, drawEnemyDeathVisual
} from './visuals/enemyVisuals.js?v=2.7.0';
...
export class Game {
  ...
  enemyCap() {
    ...
    const base = perf ? 82 : 126;
    const growth = perf ? 7.0 : 12.0;
    const hardCap = perf ? 230 : 390;
  ...
  updateSpawn(dt) {
    ...
    const interval = Math.max(.24, (.72 - Math.min(.38, minute * .019)) / (this.difficulty.spawn * waveMul * challengeMul));
    const max = this.enemyCap();

    if (this.spawnAcc >= interval && this.enemies.length < max) {
      this.spawnAcc = 0;
      let amount = 8 + Math.floor(minute / 2.0);
      ...
      amount = Math.max(8, Math.min(28, amount));
  ...
  chooseEnemyForGroup(minute, index, amount) {
    ...
    const r = Math.random();
    if (this.hordeWaveTimer > 0 && this.hordeWaveType === 'boneTide') return r < .78 ? 'skeleton' : r < .88 ? 'boneArcher' : 'summoner';
    ...
    if (minute < 5) return r < .38 ? 'slime' : r < .61 ? 'goblin' : r < .81 ? 'skeleton' : r < .95 ? 'bat' : 'boneArcher';
    if (minute < 10) return r < .38 ? 'skeleton' : r < .56 ? 'goblin' : r < .72 ? 'bat' : r < .78 ? 'boneArcher' : r < .91 ? 'orc' : 'bomber';
    if (minute < 15) return r < .34 ? 'skeleton' : r < .50 ? 'orc' : r < .64 ? 'darkMage' : r < .70 ? 'boneArcher' : r < .82 ? 'bomber' : r < .92 ? 'summoner' : 'stoneGolem';
    return r < .22 ? 'orc' : r < .40 ? 'skeleton' : r < .52 ? 'darkMage' : r < .58 ? 'boneArcher' : r < .69 ? 'bomber' : r < .79 ? 'summoner' : r < .89 ? 'demon' : r < .96 ? 'specter' : 'stoneGolem';
  }

  boneArcherCap() {
    const minute = this.time / 60;
    if (minute < 4) return 3;
    if (minute < 10) return 5;
    if (minute < 18) return 7;
    return 8;
  }
  ...
  spawnBoss(type) {
    ...
    e.maxHp *= (type === 'finalBoss' ? 1.3 : 1) * (1 + Math.min(.55, Math.max(0, cycle - 1) * .045));
    e.hp = e.maxHp;
