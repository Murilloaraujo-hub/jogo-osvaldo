import { WEAPONS, getWeaponStats } from './weaponData.js?v=2.6.0';
...
export class WeaponSystem {
  ...
  fireCompanion(id,w,level,power,kind){ const g=this.g,p=g.player,count=id==='eidolonPrime'||id==='runicOverseer'?2:Math.min(2,1+(level>=3?1:0)); g.ensureSummons(id,count,w.damage*power,false,kind,{ranged:true,companion:true,chains:id==='runicOverseer'?1:0}); }
  ...
  fireFusion(id, w, level, power) {
    ...
    if (id === 'thunderFamiliar') { g.ensureSummons(id,w.count||1,w.damage*power,true,'familiar',{ranged:true,companion:true,chains:w.chains||1,chainRange:w.range||160}); return; }
