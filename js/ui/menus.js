import { CHARACTERS, MAPS, META_UPGRADES, DIFFICULTIES, ACHIEVEMENTS } from '../config.js?v=2.9.0';
import { WEAPONS } from '../weapons/weaponData.js?v=2.9.0';
import { EVOLUTIONS, ARCANE_FUSIONS } from '../evolutions.js?v=2.9.0';
import { RELICS } from '../relics.js?v=2.9.0';
import { CLASS_PASSIVES } from '../systems/classPassives.js?v=2.9.0';
import { drawClassPreview } from '../visuals/playerVisuals.js?v=2.9.0';
import { FUSION_ITEMS } from '../items/fusionItems.js?v=2.9.0';
import { RUN_ITEMS } from '../items/itemCatalog.js?v=2.9.0';
import { SIGNATURE_ABILITIES } from '../abilities/signatures.js?v=2.9.0';
import { FUSION_RECIPES } from '../fusions.js?v=2.9.0';
import { recipeState, runRequirement, howToUnlock, fusionSearchText, signatureRows, baseForEvolution } from '../grimoire.js?v=2.9.0';
import { abilityInspection, fusionInspection, itemInspection, relicInspection, passiveInspection, runInspection, fmtNumber } from '../systems/buildInspector.js?v=2.9.0';
...
const rarityLabel = { common: 'COMUM', rare: 'RARO', epic: 'ÉPICO', legendary: 'LENDÁRIO', arcane: 'ARCANO', cursed:'AMALDIÇOADO', fusion:'FUSION' };
const escapeHtml = value => String(value ?? '').replace(/[&<>"']/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
const metricLabel = key => ({activations:'Ativações',extraProjectiles:'Projéteis extras',burnProcs:'Queimaduras aplicadas',summons:'Invocações',copies:'Cópias criadas'}[key] || key.replace(/([A-Z])/g,' $1').replace(/^./,x=>x.toUpperCase()));
...
export class UI {
  ...
  bindBase() {
    ...
    document.getElementById('pauseTabs')?.addEventListener('click', e => {
      const button = e.target.closest('[data-pause-tab]');
      if (!button) return;
      this.selectPauseTab(button.dataset.pauseTab);
    });
  ...
  updateHUD(g) {
    ...
    if (passiveHud && g.classPassives) {
      ...
      passiveHud.title = `${info.name} — ${info.active ? 'ATIVA' : `${Math.max(0, info.remaining).toFixed(1)}s`}`;
      passiveHud.setAttribute('aria-label', passiveHud.title);
      passiveHud.innerHTML = `<span class="passive-icon">${info.icon}</span><div><small>${info.active ? 'ATIVA' : `${Math.max(0, info.remaining).toFixed(1)}s`}</small></div>`;
  ...
  refreshFusionTracker(g){
    ...
    const id=g?.trackedFusionId||this.save.data.settings.trackedFusion;
    if(!id){box.classList.add('hidden');return;}
    const r=FUSION_RECIPES.find(x=>x.id===id);if(!r){box.classList.add('hidden');return;}
    const req=runRequirement(g,r),done=req.filter(x=>x.ready).length,near=done===req.length-1;
    const w=WEAPONS[r.result];
    box.classList.remove('hidden');
    box.title=`${w?.name||r.name} — ${done}/${req.length} requisitos. ESC → GRIMÓRIO para detalhes.`;
    box.setAttribute('aria-label',box.title);
    box.innerHTML=`<b>${near?'FUSION PRÓXIMA':'FUSION'} • ${w?.icon||'✦'}</b><small>${done}/${req.length} requisitos • ESC → GRIMÓRIO</small>`;
  }

  refreshItems(ids=[]){
    const box=document.getElementById('itemBar'); if(!box)return; box.innerHTML='';
    if(!ids.length){box.classList.add('hidden');return;}
    box.classList.remove('hidden');
    for(const id of ids){
      const it=RUN_ITEMS[id]; if(!it)continue;
      const d=document.createElement('div');
      d.className=`relic-chip hud-icon rarity-outline-${it.rarity||'common'}`;
      d.title=`${it.name} • ${rarityLabel[it.rarity]||it.rarity} • ${it.desc}`;
      d.setAttribute('aria-label',`${it.name}: ${it.desc}`);
  ...
  refreshRelics(ids = []) {
    ...
    for (const id of ids) {
      ...
      if (!r) continue;
      const d = document.createElement('div');
      d.className = `relic-chip hud-icon rarity-outline-${r.rarity || 'rare'}`;
      d.title = `${r.name} • ${rarityLabel[r.rarity] || r.rarity}: ${r.desc}`;
      d.setAttribute('aria-label', `${r.name}: ${r.desc}`);
      d.innerHTML = `<span>${r.icon}</span>`;
      box.appendChild(d);
    }
  }
  ...
  refreshFusionItems(items = {}) {
    ...
    for (const [id, count] of entries) {
      ...
      if (!item) continue;
      const d = document.createElement('div');
      d.className = `fusion-item-chip hud-icon rarity-outline-fusion`;
  ...
  showPause(show) {
    ...
    if(show){
      ...
      this.pauseTab=this.pauseTab||'summary';
      this.pauseSelected=null;
      this.renderPauseTab();
    }
  }
  ...
  isPauseOpen(){return !document.getElementById('pauseModal')?.classList.contains('hidden');}
  ...
  selectPauseTab(tab, selected=null){
    this.pauseTab=tab||'summary';
    this.pauseSelected=selected;
    this.renderPauseTab();
  }

  pauseEntry(icon,name,sub,onClick,classes=''){
    const b=document.createElement('button');
    b.className=`pause-entry ${classes}`;
    b.innerHTML=`<span class="pause-entry-icon">${icon||'✦'}</span><span class="pause-entry-copy"><b>${escapeHtml(name)}</b><small>${escapeHtml(sub||'')}</small></span>`;
    if(onClick)b.onclick=onClick;
    return b;
  }

  sourceDisplay(id){
    if(WEAPONS[id])return {icon:WEAPONS[id].icon,name:WEAPONS[id].name};
    if(RELICS[id])return {icon:RELICS[id].icon,name:`Relíquia • ${RELICS[id].name}`};
    if(RUN_ITEMS[id])return {icon:RUN_ITEMS[id].icon,name:`Item • ${RUN_ITEMS[id].name}`};
    if(id?.startsWith('classPassive:')){const cid=id.split(':')[1];return {icon:CLASS_PASSIVES[cid]?.icon||'✦',name:`Passiva • ${CLASS_PASSIVES[cid]?.name||cid}`};}
    if(id?.startsWith('item:')){const key=id.split(':')[1];const it=RUN_ITEMS[key];return {icon:it?.icon||'◈',name:`Item • ${it?.name||key}`};}
    const clean=String(id||'Outro').replace(/([A-Z])/g,' $1');
    return {icon:'✦',name:clean.charAt(0).toUpperCase()+clean.slice(1)};
  }

  renderPauseTab(){
    if(!this.game?.player)return;
    document.querySelectorAll('[data-pause-tab]').forEach(b=>b.classList.toggle('active',b.dataset.pauseTab===this.pauseTab));
    const list=document.getElementById('pauseBuildList'),detail=document.getElementById('pauseBuildDetail');
    if(!list||!detail)return;
    list.innerHTML='';detail.innerHTML='';
    const p=this.game.player;

    if(this.pauseTab==='summary'){
      const run=runInspection(this.game);
      const groups=[
        ['HABILIDADES',p.weaponOrder.filter(id=>!WEAPONS[id]?.fusion),id=>this.selectPauseTab('abilities',id)],
        ['FUSIONS',p.weaponOrder.filter(id=>WEAPONS[id]?.fusion),id=>this.selectPauseTab('fusions',id)],
        ['ITENS',p.items||[],id=>this.selectPauseTab('items',`item:${id}`)],
        ['RELÍQUIAS',p.relics||[],id=>this.selectPauseTab('relics',id)]
      ];
      for(const [label,ids,go] of groups){
        const h=document.createElement('div');h.className='pause-list-label';h.textContent=label;list.appendChild(h);
        if(!ids.length){const empty=document.createElement('small');empty.className='pause-empty';empty.textContent='Nenhum';list.appendChild(empty);continue;}
        for(const id of ids){const def=WEAPONS[id]||RUN_ITEMS[id]||RELICS[id];list.appendChild(this.pauseEntry(def?.icon,def?.name,WEAPONS[id]?`${WEAPONS[id].fusion?'FUSION ':''}Lv.${p.weapons[id]||''}`:rarityLabel[def?.rarity]||def?.rarity,()=>go(id)));}
      }
      detail.innerHTML=`<div class="build-hero"><span>${CHARACTERS[p.id]?.icon||'✦'}</span><div><small>RUN ATUAL</small><h2>${escapeHtml(CHARACTERS[p.id]?.name||p.id)}</h2><p>${escapeHtml(SIGNATURE_ABILITIES[p.id]?.style||'Build híbrida')}</p></div></div>
        <div class="stat-grid compact"><div><span>Tempo</span><b>${run.time}</b></div><div><span>Nível</span><b>${run.level}</b></div><div><span>HP</span><b>${run.hp}</b></div><div><span>XP</span><b>${run.xp}</b></div><div><span>Kills</span><b>${fmtNumber(run.kills)}</b></div><div><span>Bosses</span><b>${run.bosses}</b></div><div><span>Dano total</span><b>${fmtNumber(run.totalDamage)}</b></div><div><span>Cura recebida</span><b>${fmtNumber(run.healing)}</b></div></div>
        <h3>BUILD</h3><div class="build-icon-strip">${p.weaponOrder.map(id=>`<span title="${escapeHtml(WEAPONS[id]?.name)}">${WEAPONS[id]?.icon||'✦'}<small>${WEAPONS[id]?.fusion?`F${p.weapons[id]}`:WEAPONS[id]?.evolved?'E':p.weapons[id]}</small></span>`).join('')}</div>
        <p class="pause-note">Os números desta tela são lidos da run atual. Buffs de dano, área, velocidade, cooldown, quantidade, itens e relíquias já entram nos valores exibidos pelas abas de detalhes.</p>`;
      return;
    }

    if(this.pauseTab==='abilities'){
      const ids=p.weaponOrder.filter(id=>!WEAPONS[id]?.fusion);
      if(!ids.length)list.innerHTML='<p class="pause-empty">Nenhuma habilidade equipada.</p>';
      for(const id of ids){const w=WEAPONS[id];const lv=p.weapons[id];list.appendChild(this.pauseEntry(w.icon,w.name,w.evolved?'EVOLUÍDA':w.signature?`SIGNATURE • Lv.${lv}`:`Lv.${lv}`,()=>this.showPauseAbility(id,false),this.pauseSelected===id?'selected':''));}
      this.showPauseAbility(this.pauseSelected&&ids.includes(this.pauseSelected)?this.pauseSelected:ids[0],false);
      return;
    }

    if(this.pauseTab==='fusions'){
      const ids=p.weaponOrder.filter(id=>WEAPONS[id]?.fusion);
      if(!ids.length){list.innerHTML='<p class="pause-empty">Nenhuma Arcane Fusion criada nesta run.</p>';detail.innerHTML='<h2>FUSIONS</h2><p>Quando duas evoluções compatíveis e seus itens necessários forem obtidos, a Fusion substitui as duas habilidades e passa a aparecer aqui.</p>';return;}
      for(const id of ids){const w=WEAPONS[id],lv=p.weapons[id];list.appendChild(this.pauseEntry(w.icon,w.name,`FUSION Lv.${lv}/${w.max||14}`,()=>this.showPauseAbility(id,true),this.pauseSelected===id?'selected arcane':''));}
      this.showPauseAbility(this.pauseSelected&&ids.includes(this.pauseSelected)?this.pauseSelected:ids[0],true);
      return;
    }

    if(this.pauseTab==='items'){
      const entries=[];
      for(const id of p.items||[])entries.push({kind:'item',id,def:RUN_ITEMS[id]});
      for(const [id,count] of Object.entries(p.fusionItems||{}))if(count>0)entries.push({kind:'fusionItem',id,def:FUSION_ITEMS[id],count});
      if(!entries.length){list.innerHTML='<p class="pause-empty">Nenhum item obtido nesta run.</p>';detail.innerHTML='<h2>ITENS</h2><p>Itens de run e Fusion Items aparecem aqui quando forem coletados.</p>';return;}
      const normalEntries=entries.filter(e=>e.kind==='item'),fusionEntries=entries.filter(e=>e.kind==='fusionItem');
      if(normalEntries.length){const h=document.createElement('div');h.className='pause-list-label';h.textContent='RUN ITEMS';list.appendChild(h);for(const entry of normalEntries){list.appendChild(this.pauseEntry(entry.def?.icon,entry.def?.name,rarityLabel[entry.def?.rarity]||entry.def?.rarity,()=>this.showPauseItem(entry.id,false),this.pauseSelected===`item:${entry.id}`?'selected':''));}}
      if(fusionEntries.length){const h=document.createElement('div');h.className='pause-list-label';h.textContent='FUSION ITEMS';list.appendChild(h);for(const entry of fusionEntries){list.appendChild(this.pauseEntry(entry.def?.icon,entry.def?.name,`FUSION ITEM ×${entry.count}`,()=>this.showPauseItem(entry.id,true),this.pauseSelected===`fusionItem:${entry.id}`?'selected':''));}}
      const selected=entries.find(e=>`${e.kind}:${e.id}`===this.pauseSelected)||entries[0];this.showPauseItem(selected.id,selected.kind==='fusionItem');
      return;
    }

    if(this.pauseTab==='relics'){
      const ids=p.relics||[];
      if(!ids.length){list.innerHTML='<p class="pause-empty">Nenhuma relíquia obtida nesta run.</p>';detail.innerHTML='<h2>RELÍQUIAS</h2><p>Relíquias especiais aparecerão aqui com seu efeito real e contadores de uso quando aplicável.</p>';return;}
      for(const id of ids){const r=RELICS[id];list.appendChild(this.pauseEntry(r.icon,r.name,rarityLabel[r.rarity]||r.rarity,()=>this.showPauseRelic(id),this.pauseSelected===id?'selected':''));}
      this.showPauseRelic(this.pauseSelected&&ids.includes(this.pauseSelected)?this.pauseSelected:ids[0]);
      return;
    }

    if(this.pauseTab==='passive'){
      const info=passiveInspection(this.game);list.appendChild(this.pauseEntry(info?.icon,info?.name,'PASSIVA DE CLASSE',()=>this.showPausePassive(),'selected'));this.showPausePassive();return;
    }

    if(this.pauseTab==='statistics'){this.renderPauseStatistics();return;}
    if(this.pauseTab==='grimoire'){this.renderPauseGrimoire();return;}
    if(this.pauseTab==='settings'){this.renderPauseSettings();return;}
  }

  showPauseAbility(id,isFusion=false){
    if(!id)return;
    this.pauseSelected=id;
    document.querySelectorAll('#pauseBuildList .pause-entry').forEach(b=>b.classList.toggle('selected',b.textContent.includes(WEAPONS[id]?.name||'__none__')));
    const data=isFusion?fusionInspection(this.game,id):abilityInspection(this.game,id),box=document.getElementById('pauseBuildDetail');if(!data||!box)return;
    const rows=data.rows.map(r=>`<div><span>${escapeHtml(r.label)}</span><b>${escapeHtml(r.value)}</b></div>`).join('');
    const recipe=data.recipe?`<section class="detail-section"><h3>CRIADA ATRAVÉS DE</h3><div class="recipe-line">${data.recipe.abilities.map(x=>`<span>${WEAPONS[x]?.icon||'✦'} ${escapeHtml(WEAPONS[x]?.name||x)}</span>`).join('<b>+</b>')}${data.recipe.item?`<b>+</b><span>${FUSION_ITEMS[data.recipe.item]?.icon||'✦'} ${escapeHtml(FUSION_ITEMS[data.recipe.item]?.name||data.recipe.item)}</span>`:''}</div></section>`:'';
    const next=data.nextUpgrade?`<section class="detail-section next-upgrade"><span>PRÓXIMO NÍVEL</span><b>Lv.${data.level} → Lv.${data.level+1}</b><p>${data.nextUpgrade}</p></section>`:`<section class="detail-section max-level"><b>NÍVEL MÁXIMO</b></section>`;
    box.innerHTML=`<div class="detail-title"><span class="detail-icon">${data.icon}</span><div><span class="tag">${escapeHtml(data.type)}</span><h2>${escapeHtml(data.name)}</h2><p>${escapeHtml(data.element)} • Lv.${data.level}${data.max?` / ${data.max}`:''}</p></div></div><p class="detail-description">${escapeHtml(data.description)}</p>
      <div class="stat-grid">${rows}</div>
      <div class="combat-metrics"><div><span>Dano total nesta run</span><b>${fmtNumber(data.totalDamage)}</b></div><div><span>DPS recente (~5s)</span><b>${fmtNumber(data.recentDps)}</b></div><div><span>Contribuição</span><b>${data.contribution.toFixed(1)}%</b></div><div><span>Kills</span><b>${fmtNumber(data.kills)}</b></div></div>${recipe}${next}
      ${data.modifiers?.length?`<details class="detail-section"><summary>MODIFICADORES ATIVOS (${data.modifiers.length})</summary><div class="modifier-list">${data.modifiers.map(x=>`<div>${escapeHtml(x)}</div>`).join('')}</div></details>`:''}`;
  }

  showPauseItem(id,fusionItem=false){
    this.pauseSelected=`${fusionItem?'fusionItem':'item'}:${id}`;
    const data=itemInspection(this.game,id,fusionItem),box=document.getElementById('pauseBuildDetail');if(!data||!box)return;
    const usage=Object.entries(data.usage||{}).map(([k,v])=>`<div><span>${escapeHtml(metricLabel(k))}</span><b>${fmtNumber(v)}</b></div>`).join('');
    box.innerHTML=`<div class="detail-title"><span class="detail-icon">${data.icon}</span><div><span class="tag rarity-${data.rarity}">${escapeHtml(rarityLabel[data.rarity]||data.rarity)}</span><h2>${escapeHtml(data.name)}</h2><p>${escapeHtml(data.type)}</p></div></div><p class="detail-description">${escapeHtml(data.description)}</p><div class="stat-grid compact"><div><span>Stacks</span><b>${data.stacks}</b></div>${usage}</div>${data.source?`<section class="detail-section"><h3>OBTENÇÃO</h3><p>Fonte principal: <b>${escapeHtml(ENEMY_LABEL(data.source))}</b></p></section>`:''}${data.uses?.length?`<section class="detail-section"><h3>USADO EM</h3>${data.uses.map(x=>`<div class="info-row"><span>${escapeHtml(x)}</span></div>`).join('')}</section>`:''}`;
  }

  showPauseRelic(id){
    this.pauseSelected=id;const data=relicInspection(this.game,id),box=document.getElementById('pauseBuildDetail');if(!data||!box)return;const usage=Object.entries(data.usage||{}).map(([k,v])=>`<div><span>${escapeHtml(metricLabel(k))}</span><b>${fmtNumber(v)}</b></div>`).join('');box.innerHTML=`<div class="detail-title"><span class="detail-icon">${data.icon}</span><div><span class="tag rarity-${data.rarity}">${escapeHtml(rarityLabel[data.rarity]||data.rarity)}</span><h2>${escapeHtml(data.name)}</h2><p>${escapeHtml(data.type)}</p></div></div><p class="detail-description">${escapeHtml(data.description)}</p>${usage?`<div class="stat-grid compact">${usage}</div>`:'<p class="pause-note">Esta relíquia modifica a build continuamente e não possui contador de ativação separado.</p>'}`;
  }

  showPausePassive(){
    const data=passiveInspection(this.game),box=document.getElementById('pauseBuildDetail');if(!data||!box)return;const runtime=Object.entries(data.runtime||{}).map(([k,v])=>`<div><span>${escapeHtml(k)}</span><b>${escapeHtml(v)}</b></div>`).join('');box.innerHTML=`<div class="detail-title"><span class="detail-icon">${data.icon}</span><div><span class="tag">PASSIVA DE CLASSE</span><h2>${escapeHtml(data.name)}</h2><p>${escapeHtml(CHARACTERS[this.game.player.id]?.name||this.game.player.id)}</p></div></div><p class="detail-description">${escapeHtml(data.description)}</p><div class="stat-grid"><div><span>Cooldown</span><b>${data.cooldown.toFixed(1)}s</b></div><div><span>Próxima ativação</span><b>${data.remaining.toFixed(1)}s</b></div><div><span>Ativações</span><b>${fmtNumber(data.activations)}</b></div><div><span>Dano total</span><b>${fmtNumber(data.totalDamage)}</b></div><div><span>DPS recente</span><b>${fmtNumber(data.recentDps)}</b></div>${runtime}</div>`;
  }

  renderPauseStatistics(){
    const run=runInspection(this.game),list=document.getElementById('pauseBuildList'),box=document.getElementById('pauseBuildDetail');list.innerHTML='<div class="pause-list-label">DAMAGE BREAKDOWN</div>';const entries=run.entries.slice(0,16);for(const [id,value] of entries){const src=this.sourceDisplay(id),pct=run.total?value/run.total*100:0;list.appendChild(this.pauseEntry(src.icon,src.name,`${pct.toFixed(1)}% • ${fmtNumber(value)}`,null));}
    box.innerHTML=`<h2>ESTATÍSTICAS DA RUN</h2><div class="stat-grid"><div><span>Tempo</span><b>${run.time}</b></div><div><span>Nível</span><b>${run.level}</b></div><div><span>HP</span><b>${run.hp}</b></div><div><span>XP</span><b>${run.xp}</b></div><div><span>Kills</span><b>${fmtNumber(run.kills)}</b></div><div><span>Elites</span><b>${run.elites}</b></div><div><span>Bosses</span><b>${run.bosses}</b></div><div><span>Dano total</span><b>${fmtNumber(run.totalDamage)}</b></div><div><span>Maior crítico</span><b>${fmtNumber(run.highestCrit)}</b></div><div><span>Cura recebida</span><b>${fmtNumber(run.healing)}</b></div><div><span>Moedas</span><b>${fmtNumber(run.coins)}</b></div></div><h3>DAMAGE BREAKDOWN</h3><div class="damage-breakdown pause-damage">${entries.map(([id,value])=>{const src=this.sourceDisplay(id),pct=run.total?value/run.total*100:0;return `<div class="damage-row"><span>${src.icon} ${escapeHtml(src.name)}</span><b>${pct.toFixed(1)}%</b><div class="mini-bar"><span style="width:${Math.min(100,pct)}%"></span></div></div>`;}).join('')||'<p>O dano aparecerá aqui após o primeiro acerto.</p>'}</div>`;
  }

  renderPauseGrimoire(){
    const list=document.getElementById('pauseBuildList'),box=document.getElementById('pauseBuildDetail'),mode=this.save.data.settings.recipeMode||'discovery';
    const scored=FUSION_RECIPES.map(r=>{const req=runRequirement(this.game,r),done=req.filter(x=>x.ready).length;return {r,req,done,total:req.length,state:recipeState(this.save,r)};}).sort((a,b)=>(b.done/Math.max(1,b.total))-(a.done/Math.max(1,a.total))||a.r.name.localeCompare(b.r.name));
    for(const x of scored){const reveal=mode==='all'||x.state==='discovered',partial=x.state==='partial'||x.done>0,name=reveal?(WEAPONS[x.r.result]?.name||x.r.name):partial?'RECEITA PARCIAL':'???',icon=reveal?(WEAPONS[x.r.result]?.icon||'✦'):'🔒';list.appendChild(this.pauseEntry(icon,name,`${x.done}/${x.total} requisitos`,()=>this.showPauseRecipe(x.r),this.pauseSelected===x.r.id?'selected':''));}
    const first=scored.find(x=>x.r.id===this.pauseSelected)?.r||scored[0]?.r;if(first)this.showPauseRecipe(first);else box.innerHTML='<p>Nenhuma receita disponível.</p>';
  }

  showPauseRecipe(r){
    this.pauseSelected=r.id;const box=document.getElementById('pauseBuildDetail'),req=runRequirement(this.game,r),state=recipeState(this.save,r),reveal=this.save.data.settings.recipeMode==='all'||state==='discovered';const result=WEAPONS[r.result];const how=howToUnlock(r);box.innerHTML=`<div class="detail-title"><span class="detail-icon">${reveal?result?.icon||'✦':'🔒'}</span><div><span class="tag">ARCANE GRIMOIRE</span><h2>${reveal?escapeHtml(result?.name||r.name):'RECEITA NÃO DESCOBERTA'}</h2><p>MAX Lv.${result?.max||14}${r.classId?` • Exclusiva: ${escapeHtml(CHARACTERS[r.classId]?.name||r.classId)}`:''}</p></div></div><p>${reveal?escapeHtml(result?.desc||''):escapeHtml(r.hint||'Descubra evoluções e itens relacionados para revelar a receita.')}</p><h3>REQUISITOS DA RUN</h3><div class="requirements-grid">${req.map(x=>`<div class="requirement ${x.ready?'ready':''}"><b>${x.ready?'✓':'✗'} ${escapeHtml(x.label)}</b>${x.baseLv?`<small>Progresso: Lv.${x.baseLv}/5</small>`:''}${x.source?`<small>Fonte principal: ${escapeHtml(ENEMY_LABEL(x.source))}</small>`:''}</div>`).join('')}</div><button id="pauseTrackFusion">${this.save.data.settings.trackedFusion===r.id?'PARAR DE RASTREAR':'RASTREAR FUSION'}</button><details class="detail-section"><summary>COMO CONSEGUIR?</summary><ol>${how.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ol></details>`;box.querySelector('#pauseTrackFusion').onclick=()=>{this.toggleTrackFusion(r.id);this.showPauseRecipe(r);};
  }

  renderPauseSettings(){
    const list=document.getElementById('pauseBuildList'),box=document.getElementById('pauseBuildDetail'),s=this.save.data.settings;list.innerHTML='<div class="pause-list-label">CONFIGURAÇÕES RÁPIDAS</div><p class="pause-empty">Alterações são salvas automaticamente.</p>';box.innerHTML='<h2>CONFIGURAÇÕES</h2><div id="pauseQuickSettings" class="settings-grid pause-settings"></div>';const target=box.querySelector('#pauseQuickSettings');for(const [key,label] of [['damageNumbers','Números de dano'],['shake','Screen shake'],['performance','Modo performance'],['hitboxes','Mostrar hitboxes'],['minimapEnabled','Minimapa']]){const row=document.createElement('label');row.className='setting';row.innerHTML=`<span>${label}</span><input type="checkbox" ${s[key]?'checked':''}>`;row.querySelector('input').onchange=e=>{s[key]=e.target.checked;this.save.write();};target.appendChild(row);}const quality=document.createElement('label');quality.className='setting';quality.innerHTML='<span>Qualidade dos efeitos</span><select><option value="0">Baixa</option><option value="1">Média</option><option value="2">Alta</option></select>';quality.querySelector('select').value=String(s.quality??2);quality.querySelector('select').onchange=e=>{s.quality=+e.target.value;this.save.write();};target.appendChild(quality);
  }
