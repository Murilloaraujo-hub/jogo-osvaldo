import { WEAPONS, getWeaponStats } from '../js/weapons/weaponData.js?v=2.6.0';
import { EVOLUTIONS, availableTransformations, replaceAbility, autoEvolutionFor } from '../js/evolutions.js?v=2.6.0';
import { FUSION_RECIPES } from '../js/fusions.js?v=2.6.0';

function playerWith(ids){
  const weapons={};
  for(const id of ids) weapons[id]=1;
  return {weapons,weaponOrder:[...ids]};
}
...
for(const recipe of FUSION_RECIPES){
  const p=playerWith(recipe.abilities);
  ...
  // Fusion Lv.1-5 must materially change stats/config at each step.
  let prev=null;
