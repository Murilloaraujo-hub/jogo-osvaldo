...
const COLORS={player:'#ffffff',chest:'#ffd465',elite:'#ffb347',miniboss:'#ff7998',boss:'#ff3f8e',altar:'#ffe18a',merchant:'#83e6b7',portal:'#c78cff',challenge:'#ffda78',cursed:'#ef6c9a',poi:'#d4e5d9'};
...
export class MinimapSystem{
  ...
  collectObjectives(){
    ...
    for(const p of g.pickups||[])if(p.type==='chest')arr.push({type:'chest',x:p.x,y:p.y,label:'BAÚ',priority:2});
    ...
    for(const e of g.enemies||[]){if(e.dead)continue;if(e.type==='finalBoss')arr.push({type:'boss',x:e.x,y:e.y,label:'BOSS',priority:10});else if(e.boss)arr.push({type:'miniboss',x:e.x,y:e.y,label:'MINIBOSS',priority:8});else if(e.elite)arr.push({type:'elite',x:e.x,y:e.y,label:'ELITE',priority:1});}
    return arr;
  }
  ...
  drawMarkers(c,css,pad,range){
    ...
    const edge=[];
    for(const o of objectives){const p=this.worldToMini(o.x,o.y,css,pad,range);if(this.insideMini(p,css,pad,4)){marker(c,o.type,p.x,p.y,o.type==='boss'?5.8:o.type==='miniboss'?4.8:o.type==='elite'?2.6:3.5);}else if(o.priority>=2){edge.push(o);}}
