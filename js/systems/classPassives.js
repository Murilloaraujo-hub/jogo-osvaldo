import { ENEMIES } from '../config.js?v=2.9.0';
...
export const CLASS_PASSIVES = {
  ...
  summoner: { icon:'🪬', name:'Familiar Ancestral', cooldown:8, desc:'Mantém uma luz orbital; a cada 8s cria cópias luminosas que disparam uma barragem e retornam ao núcleo.' },
  ...
  technomancer: { icon:'⚙️', name:'Drone Arcano', cooldown:7, desc:'Mantém drones rúnicos; a cada 7s eles alinham alvos, executam uma barragem e encerram com um laser.' }
...
export class ClassPassiveSystem {
  ...
  markActivation() {
    this.activations = (this.activations || 0) + 1;
    this.lastActivation = this.g.time || 0;
  }

  getRuntimeStats() {
    const g=this.g,p=g.player,tier=Math.floor((p.level-1)/5);
    switch(this.id){
      case 'mage': return { 'Dano estimado por feixe': Math.round((70+tier*10)*p.damage), 'Largura': Math.round(20*(1+Math.min(.55,tier*.12))), 'Alcance': 1080+Math.min(250,tier*55), 'Feixes secundários': Math.min(2,Math.floor(p.level/8)) };
      case 'necromancer': return { 'Mortes registradas': this.necroRegistry.length, 'Aliados temporários': g.summons.filter(s=>s.classPassiveFallen&&!s.dead).length, 'Duração dos ressuscitados': '20s' };
      case 'archer': return { 'Flechas por chuva': 14+Math.min(18,Math.floor(p.level/2)), 'Dano por impacto': Math.round(22*p.damage*(1+Math.min(.8,p.level*.025))), 'Área atual': Math.round(95+Math.min(45,p.level*1.4)) };
      case 'knight': return { 'Redução durante barreira': '52%', 'Duração': '2s', 'Dano da explosão': Math.round(58*p.damage), 'Raio': Math.round(138*p.area) };
      case 'druid': return { 'Dano das raízes': Math.round(48*p.damage*(1+Math.min(.65,p.level*.02))), 'Controle': '1,05s', 'Chance de Flor Vital': '18%' };
      case 'assassin': return { 'Dano do corte': Math.round(78*p.damage), 'Bônus de crítico': '+28%', 'Alcance de busca': 1000 };
      case 'warlock': return { 'Alvos marcados': Math.min(10,5+Math.floor(p.level/5)), 'Duração da marca': '8s', 'Dano da explosão': Math.round(42*p.damage) };
      case 'paladin': return { 'Dano da coluna': Math.round(72*p.damage), 'Redução da barreira': '28%', 'Duração da barreira': '1,4s' };
      case 'elementalist': return { 'Elementos por ciclo': Math.min(4,1+Math.floor(p.level/7)), 'Ciclo': 'Fogo → Gelo → Raio → Terra' };
      case 'battlemage': return { 'Dano do corte': Math.round(82*p.damage), 'Alcance': 760, 'Cortes extras': Math.min(2,Math.floor(p.level/8)) };
      case 'summoner': return { 'Alvos da barragem': 6, 'Familiar passivo ativo': g.summons.some(s=>s.id==='classFamiliar'&&!s.dead)?'Sim':'Não' };
      case 'bloodMage': return { 'Dano da Nova': Math.round((52+p.maxHp*.12)*p.damage), 'Raio': Math.round(145*p.area), 'Lanças extras': Math.min(8,3+Math.floor(p.level/5)) };
      case 'monk': return { 'Dano do golpe': Math.round(58*p.damage), 'Golpes por ativação': Math.min(4,1+Math.floor(p.level/6)), 'Alcance': 360 };
      case 'technomancer': return { 'Alvos travados': Math.min(9,4+Math.floor(p.level/5)), 'Drone passivo ativo': g.summons.some(s=>s.id==='classDrone'&&!s.dead)?'Sim':'Não' };
      default:return {};
    }
  }
  ...
  update(dt) {
    ...
    switch (this.id) {
      ...
      case 'summoner': this.activateSummonerBurst(); break;
      case 'bloodMage': this.activateBloodMage(); break;
      case 'monk': this.activateMonk(); break;
      case 'technomancer': this.activateTechnomancerBarrage(); break;
  ...
  fireMageLaser(state, allowSecond = true) {
    ...
    this.effects.push({ kind: 'mageLaser', ax, ay, bx, by, width, t: .24, max: .24 });
    const extra=Math.min(2,Math.floor(p.level/8));for(let i=0;i<extra;i++){const aa=state.angle+(i?-.16:.16);const ebx=ax+Math.cos(aa)*range,eby=ay+Math.sin(aa)*range;for(const e of g.enemies){if(!e.dead&&pointSegmentDistance(e.x,e.y,ax,ay,ebx,eby)<=(e.hitboxRadius||e.size)+width*.32)g.damageEnemy(e,damage*.42,false,'arcane',true,'classPassive:mage');}this.effects.push({kind:'mageLaser',ax,ay,bx:ebx,by:eby,width:width*.5,t:.18,max:.18});}
  ...
  fireArrowRain(state) {
    const g = this.g, p = g.player;
    const count = 14 + Math.min(18, Math.floor(p.level / 2));
  ...
  activateWarlock() {
    const g=this.g,p=g.player, targets=g.closestEnemies(p.x,p.y,900,Math.min(10,5+Math.floor(p.level/5)));
    for (const e of targets) { e.darkPactTimer=8; e.darkPactMarked=true; }
    if (targets.length) { this.markActivation(); this.effects.push({kind:'darkPact', x:p.x,y:p.y,r:150,t:.6,max:.6}); }
    this.timer=this.info.cooldown;
  }
  ...
  activatePaladin() {
    ...
    this.markActivation();
    const r=(145+Math.min(55,p.level*2))*p.area; g.areaHit(t.x,t.y,r,72*p.damage,'holy',0,false,'classPassive:paladin');
    ...
    this.effects.push({kind:'divineJudgment',x:t.x,y:t.y,r,t:.72,max:.72}); if(p.level>=10){for(let i=0;i<2;i++){const a=Math.random()*Math.PI*2,x=t.x+Math.cos(a)*r*.55,y=t.y+Math.sin(a)*r*.55;g.delayedEffects.push({t:.1+i*.12,fn:()=>{g.areaHit(x,y,r*.42,40*p.damage,'holy',0,false,'classPassive:paladin');this.effects.push({kind:'divineJudgment',x,y,r:r*.42,t:.42,max:.42});}});}}
    this.timer=this.info.cooldown;
  }
  ...
  activateElementalist() {
    ...
    this.markActivation();
    const elements=['fire','ice','electric','earth']; const base=this.elementIndex++%elements.length; const sequence=Math.min(4,1+Math.floor(p.level/7));
    for(let j=0;j<sequence;j++){const el=elements[(base+j)%elements.length],delay=j*.09;g.delayedEffects.push({t:delay,fn:()=>{if(el==='electric'){const q=this.chooseCluster(900)||t;g.damageEnemy(q,68*p.damage,true,'electric',true,'classPassive:elementalist');g.lightningArcs.push({x1:p.x,y1:p.y,x2:q.x,y2:q.y,life:.16,color:g.colorFor('electric')});}else if(el==='earth'){g.areaHit(t.x,t.y,98,70*p.damage,'earth',0,false,'classPassive:elementalist');g.addAbilityEffect({type:'physicalImpact',x:t.x,y:t.y,radius:98,life:.35,maxLife:.35});}else{g.areaHit(t.x,t.y,96,64*p.damage,el,el==='ice'?1.3:0,false,'classPassive:elementalist');g.addAbilityEffect({type:el==='fire'?'fireExplosion':'iceShatter',x:t.x,y:t.y,radius:96,life:.35,maxLife:.35});}this.effects.push({kind:'elementCycle',x:t.x,y:t.y,r:96,element:el,t:.48,max:.48});}});}
    this.timer=this.info.cooldown;
  }
  ...
  activateBattleMage() {
    ...
    this.effects.push({kind:'battleSlash',ax:p.x,ay:p.y,bx,by,width,t:.3,max:.3}); const extras=Math.min(2,Math.floor(p.level/8));for(let i=0;i<extras;i++){const aa=angle+(i?-.22:.22),ex=p.x+Math.cos(aa)*range,ey=p.y+Math.sin(aa)*range;g.delayedEffects.push({t:.1+i*.08,fn:()=>{for(const e of g.enemies){if(!e.dead&&pointSegmentDistance(e.x,e.y,p.x,p.y,ex,ey)<=(e.hitboxRadius||e.size)+width*.4)g.damageEnemy(e,48*p.damage,false,'arcane',true,'classPassive:battlemage');}this.effects.push({kind:'battleSlash',ax:p.x,ay:p.y,bx:ex,by:ey,width:width*.72,t:.22,max:.22});}});} this.timer=this.info.cooldown;
  }

  activateSummonerBurst() {
    this.markActivation();
    const g=this.g,p=g.player;this.ensurePassiveCompanion('classFamiliar');const targets=g.closestEnemies(p.x,p.y,800,6);for(let i=0;i<targets.length;i++){const t=targets[i],a=i*Math.PI*2/Math.max(1,targets.length),x=p.x+Math.cos(a)*68,y=p.y+Math.sin(a)*68;this.effects.push({kind:'summonerBurst',x,y,x2:t.x,y2:t.y,t:.42,max:.42});g.delayedEffects.push({t:.08+i*.035,fn:()=>g.damageEnemy(t,32*p.damage,true,'arcane',true,'classPassive:summoner')});}this.timer=this.info.cooldown;
  }
  ...
  activateBloodMage() {
    ...
    this.effects.push({kind:'bloodNova',x:p.x,y:p.y,r,t:.62,max:.62}); const targets=g.closestEnemies(p.x,p.y,720,Math.min(8,3+Math.floor(p.level/5)));for(let i=0;i<targets.length;i++){const q=targets[i];g.delayedEffects.push({t:.08+i*.035,fn:()=>{g.damageEnemy(q,26*p.damage,false,'shadow',true,'classPassive:bloodMage');g.addAbilityEffect({type:'shadowBurst',x:q.x,y:q.y,radius:34,life:.2,maxLife:.2});}});} this.timer=this.info.cooldown;
  }
  ...
  activateMonk() {
    ...
    this.effects.push({kind:'kiBurst',ax:p.x,ay:p.y,bx,by,width,t:.34,max:.34}); const hits=Math.min(4,1+Math.floor(p.level/6));for(let i=1;i<hits;i++)g.delayedEffects.push({t:i*.07,fn:()=>{const aa=a+(i%2?-.14:.14),ex=p.x+Math.cos(aa)*range,ey=p.y+Math.sin(aa)*range;for(const e of g.enemies){if(!e.dead&&pointSegmentDistance(e.x,e.y,p.x,p.y,ex,ey)<=(e.hitboxRadius||e.size)+width*.35)g.damageEnemy(e,30*p.damage,false,'wind',true,'classPassive:monk');}this.effects.push({kind:'kiBurst',ax:p.x,ay:p.y,bx:ex,by:ey,width:width*.65,t:.22,max:.22});}}); this.timer=this.info.cooldown;
  }

  activateTechnomancerBarrage() {
    this.markActivation();
    const g=this.g,p=g.player;this.ensurePassiveCompanion('classDrone');const targets=g.closestEnemies(p.x,p.y,900,Math.min(9,4+Math.floor(p.level/5)));let prev={x:p.x,y:p.y};for(let i=0;i<targets.length;i++){const t=targets[i];g.delayedEffects.push({t:i*.045,fn:()=>{g.damageEnemy(t,30*p.damage,true,'arcane',true,'classPassive:technomancer');g.lightningArcs.push({x1:prev.x,y1:prev.y,x2:t.x,y2:t.y,life:.1,color:'#84e8ff'});prev=t;}});}const main=targets[0];if(main)g.delayedEffects.push({t:.36,fn:()=>{const a=Math.atan2(main.y-p.y,main.x-p.x),range=800,bx=p.x+Math.cos(a)*range,by=p.y+Math.sin(a)*range;for(const e of g.enemies){if(!e.dead&&pointSegmentDistance(e.x,e.y,p.x,p.y,bx,by)<=(e.hitboxRadius||e.size)+15)g.damageEnemy(e,52*p.damage,false,'arcane',true,'classPassive:technomancer');}this.effects.push({kind:'battleSlash',ax:p.x,ay:p.y,bx,by,width:24,t:.2,max:.2});}});this.timer=this.info.cooldown;
  }
  ...
  raiseFallen() {
    ...
    if (picked.length) this.markActivation();
  ...
  renderEffect(ctx, e) {
    ...
    } else if (e.kind === 'summonerBurst') { const a=screen(g,e.x,e.y),b=screen(g,e.x2,e.y2),fade=Math.max(0,e.t/e.max);ctx.globalAlpha=fade;ctx.strokeStyle='#d8b8ff';ctx.lineWidth=2;ctx.beginPath();ctx.arc(a.x,a.y,8,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();ctx.globalAlpha=1;
