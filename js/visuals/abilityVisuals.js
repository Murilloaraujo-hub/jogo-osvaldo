...
export function drawSummonVisual(ctx, game, summon) {
  ...
  if (summon.orbitalLight) {
    ctx.save();
    ctx.translate(s.x, s.y);
    const electric = summon.electric || summon.id === 'thunderFamiliar';
    const solar = summon.variant === 'fire' || summon.id === 'solarFamiliar';
    const color = solar ? '#ffd15a' : electric ? '#ffe45b' : '#d9c9ff';
    withGlow(ctx, q, color, summon.doubleRing ? 18 : 12);
    ctx.fillStyle = color;
    ctx.beginPath(); ctx.arc(0, 0, summon.doubleRing ? 7 : 5.5, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = electric ? '#fff3a6' : '#ffffffcc';
    ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.ellipse(0,0,summon.doubleRing?13:10,summon.doubleRing?6:5,game.time*1.8,0,Math.PI*2); ctx.stroke();
    if (summon.doubleRing) {
      ctx.beginPath(); ctx.ellipse(0,0,16,7,-game.time*1.3,0,Math.PI*2); ctx.stroke();
    }
    if (q >= 1) {
      for (let i=0;i<3;i++){const a=game.time*2.2+i*Math.PI*2/3;ctx.fillStyle=color;ctx.globalAlpha=.6;ctx.beginPath();ctx.arc(Math.cos(a)*14,Math.sin(a)*14,1.6,0,Math.PI*2);ctx.fill();}
    }
    ctx.restore();
    return;
  }
...
export function drawPersistentAbilities(ctx, game) {
  ...
  if (has('elementalBolt') || has('primalConvergence')) {
    const rr = has('primalConvergence') ? 42 : 34;
    const a = t * 1.25;
    ctx.save();
    ctx.translate(Math.cos(a)*rr, Math.sin(a)*rr);
    ctx.rotate(-t*.8);
    ctx.strokeStyle = '#d6c8ffcc';
    ctx.lineWidth = 1.6;
    runeDiamond(ctx, 0, 0, has('primalConvergence') ? 9 : 7, -t*.8);
    if (q >= 1) {
      const cols=['#ff7043','#70d8ff','#ffe45b','#c59a65'];
      ctx.fillStyle=cols[Math.floor(t*1.2)%4];
