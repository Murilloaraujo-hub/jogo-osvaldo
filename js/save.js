...
function migrate(data) {
  ...
  if (!Array.isArray(migrated.abilities)) migrated.abilities = [];
  if (!Array.isArray(migrated.evolutions)) migrated.evolutions = [];
  if (!Array.isArray(migrated.fusionItemsDiscovered)) migrated.fusionItemsDiscovered = [];
  if (!Array.isArray(migrated.bossesDiscovered)) migrated.bossesDiscovered = [];
  if (!Array.isArray(migrated.fusions)) migrated.fusions = [];
  if (!Array.isArray(migrated.achievements)) migrated.achievements = [];
  if (migrated.settings.minimapEnabled === undefined) migrated.settings.minimapEnabled = true;
  if (migrated.settings.minimapSize === undefined) migrated.settings.minimapSize = 1;
  if (!['discovery','all'].includes(migrated.settings.recipeMode)) migrated.settings.recipeMode='discovery';
  if (migrated.settings.trackedFusion === undefined) migrated.settings.trackedFusion=null;
  return migrated;
}
...
export const Save = {
  ...
  refreshUnlocks() {
    ...
    if (r.totalKills >= 500) add('warlock');
    if (r.miniboss) add('paladin');
    if ((this.data.evolutions || []).length >= 3) add('elementalist');
    if (r.highestLevel >= 10) add('battlemage');
    if (r.totalKills >= 1500) add('summoner');
    if (r.bestTime >= 600) add('bloodMage');
    if (r.wins >= 1) add('monk');
    if ((this.data.fusions || []).length >= 3) add('technomancer');
  },
  ...
  discoverAbility(id) {
    if (!id) return;
    if (!this.data.abilities.includes(id)) { this.data.abilities.push(id); this.write(); }
  },

  discoverFusionItem(id) {
    if (!id) return;
    if (!this.data.fusionItemsDiscovered.includes(id)) { this.data.fusionItemsDiscovered.push(id); this.write(); }
  },

  discoverBoss(id) {
    if (!id) return;
    if (!this.data.bossesDiscovered.includes(id)) { this.data.bossesDiscovered.push(id); this.write(); }
  },
