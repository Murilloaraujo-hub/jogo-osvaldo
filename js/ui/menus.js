import { CHARACTERS, MAPS, META_UPGRADES, DIFFICULTIES, ACHIEVEMENTS } from '../config.js?v=2.7.0';
import { WEAPONS } from '../weapons/weaponData.js?v=2.7.0';
import { EVOLUTIONS, ARCANE_FUSIONS } from '../evolutions.js?v=2.7.0';
import { RELICS } from '../relics.js?v=2.7.0';
import { CLASS_PASSIVES } from '../systems/classPassives.js?v=2.7.0';
import { drawClassPreview } from '../visuals/playerVisuals.js?v=2.7.0';
...
export class UI {
  ...
  renderGrimoire() {
    ...
    for (const e of rows) {
      ...
      const recipeText = e.arcane && e.abilities
        ? `${WEAPONS[e.abilities[0]]?.name || '???'} + ${WEAPONS[e.abilities[1]]?.name || '???'}${e.item ? ` + ${FUSION_ITEMS[e.item]?.icon || '✦'} ${FUSION_ITEMS[e.item]?.name || e.item}` : ''} → ${WEAPONS[e.result]?.name || e.name} • Lv. máx. ${WEAPONS[e.result]?.max || 14}`
        : e.desc;
      d.innerHTML = e.known
        ? `<span class="tag">${e.arcane ? 'ARCANE FUSION' : 'EVOLUÇÃO'}</span><h3>${WEAPONS[e.result]?.icon || '✦'} ${e.name}</h3><div class="small">${recipeText}</div>`
        : `<span class="tag">${e.arcane ? 'ARCANE FUSION' : 'EVOLUÇÃO'}</span><h3>???</h3><div class="small">${e.arcane && e.abilities ? `??? + ${WEAPONS[e.abilities[1]]?.name || '???'} → ???` : 'Evolução ainda não descoberta.'}</div>`;
      box.appendChild(d);
    }
  }
