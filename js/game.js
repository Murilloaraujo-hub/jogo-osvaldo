import {
  ...
  timeHpMultiplier
} from './config.js?v=2.4.0';
import { Player } from './entities/player.js?v=2.4.0';
import { Enemy } from './entities/enemy.js?v=2.4.0';
import { Camera } from './camera.js?v=2.4.0';
import { SpatialGrid } from './systems/spatialGrid.js?v=2.4.0';
import { ObjectPool } from './systems/objectPool.js?v=2.4.0';
import { WeaponSystem, WEAPONS } from './weapons/weapons.js?v=2.4.0';
import { getWeaponLevelDescription } from './weapons/weaponData.js?v=2.4.0';
import { availableTransformations, replaceAbility, autoEvolutionFor, evolutionForBase } from './evolutions.js?v=2.4.0';
import { RELICS, rollRelic } from './relics.js?v=2.4.0';
import { checkAchievements } from './achievements.js?v=2.4.0';
import { ClassPassiveSystem, drawFallenAllyVisual } from './systems/classPassives.js?v=2.4.0';
import { WorldMap } from './world/worldMap.js?v=2.4.0';
import { MinimapSystem } from './systems/minimap.js?v=2.4.0';
import { drawPlayerVisual } from './visuals/playerVisuals.js?v=2.4.0';
import {
...
} from './visuals/abilityVisuals.js?v=2.4.0';
import {
  drawEnemyVisual, drawEnemyProjectileVisual, drawEnemyDeathVisual
} from './visuals/enemyVisuals.js?v=2.4.0';
...
export class Game {
  ...
  enemyCap() {
    ...
    const base = perf ? 62 : 92;
    const growth = perf ? 5.6 : 8.8;
    const hardCap = perf ? 175 : 285;
  ...
  updateSpawn(dt) {
    ...
    if (this.time > 45 && this.hordeWaveCooldown <= 0) {
      this.hordeWaveTimer = 8 + Math.min(5, this.time / 300);
      this.hordeWaveCooldown = 72 + Math.random() * 34;
      this.ui.flashMessage('⚠ HORDA SE APROXIMANDO');
    }
    ...
    if (this.spawnAcc >= interval && this.enemies.length < max) {
      this.spawnAcc = 0;
      let amount = 4 + Math.floor(minute / 2.6);
      if (minute >= 5) amount += 2;
      if (minute >= 10) amount += 2;
      if (minute >= 15) amount += 2;
      if (this.hordeWaveTimer > 0) amount += 5;
      if (this.challenge?.active) amount += 3;
      amount = Math.max(4, Math.min(18, amount));
  ...
  chooseEnemyForGroup(minute, index, amount) {
    ...
    return r < .24 ? 'orc' : r < .42 ? 'skeleton' : r < .56 ? 'darkMage' : r < .68 ? 'boneArcher' : r < .80 ? 'bomber' : r < .91 ? 'summoner' : 'stoneGolem';
  }
  ...
  checkTransformations() {
    ...
    // Arcane Fusions continuam sendo recompensas especiais quando os requisitos
    // das formas evoluídas/base restantes são atendidos.
    const fusions = availableTransformations(this.player);
    for (const recipe of fusions) {
      const key = `ready:${recipe.id}`;
      if (!this.flags[key]) {
        this.flags[key] = true;
        this.ui.flashMessage(`ARCANE FUSION pronta: ${recipe.name}`);
      }
    }
  }
