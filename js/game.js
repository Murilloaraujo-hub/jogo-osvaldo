import {
  ...
  timeHpMultiplier
} from './config.js?v=2.9.0';
import { Player } from './entities/player.js?v=2.9.0';
import { Enemy } from './entities/enemy.js?v=2.9.0';
import { Camera } from './camera.js?v=2.9.0';
import { SpatialGrid } from './systems/spatialGrid.js?v=2.9.0';
import { ObjectPool } from './systems/objectPool.js?v=2.9.0';
import { WeaponSystem, WEAPONS } from './weapons/weapons.js?v=2.9.0';
import { getWeaponLevelDescription } from './weapons/weaponData.js?v=2.9.0';
import { availableTransformations, replaceAbility, autoEvolutionFor, evolutionForBase, ARCANE_FUSIONS } from './evolutions.js?v=2.9.0';
import { FUSION_ITEMS } from './items/fusionItems.js?v=2.9.0';
import { RUN_ITEMS, rollRunItem, applyRunItem } from './items/itemCatalog.js?v=2.9.0';
import { abilityBias } from './abilities/signatures.js?v=2.9.0';
import { RELICS, rollRelic } from './relics.js?v=2.9.0';
import { checkAchievements } from './achievements.js?v=2.9.0';
import { ClassPassiveSystem, drawFallenAllyVisual } from './systems/classPassives.js?v=2.9.0';
import { WorldMap } from './world/worldMap.js?v=2.9.0';
import { MinimapSystem } from './systems/minimap.js?v=2.9.0';
import { drawPlayerVisual } from './visuals/playerVisuals.js?v=2.9.0';
import {
...
} from './visuals/abilityVisuals.js?v=2.9.0';
import {
  drawEnemyVisual, drawEnemyProjectileVisual, drawEnemyDeathVisual
} from './visuals/enemyVisuals.js?v=2.9.0';
...
export class Game {
  ...
  start(charId, mapId, difficulty = 'normal') {
    ...
    this.save.discoverAbility?.(this.ui.characters[charId].startWeapon);
    ...
    this.castLog = {};
    this.regenTimer = 0;
    this.itemAuraTimer = 0;
    this.trackedFusionId = this.save.data.settings.trackedFusion || null;
    ...
    this.ui.refreshFusionItems?.(this.player.fusionItems);
    this.ui.refreshItems?.(this.player.items);
  ...
  loop(t, token) {
    ...
    if (this.input.consume('Escape')) {
      const handled = this.ui.handleGameEscape?.(this);
      if (!handled && (!this.paused || this.ui.isPauseOpen?.())) this.togglePause();
    }
  ...
  enemyCap() {
    ...
    const base = perf ? 94 : 142;
    const growth = perf ? 7.8 : 13.8;
    const hardCap = perf ? 250 : 430;
  ...
  updateSpawn(dt) {
    ...
    const interval = Math.max(.21, (.63 - Math.min(.35, minute * .018)) / (this.difficulty.spawn * waveMul * challengeMul));
    const max = this.enemyCap();

    if (this.spawnAcc >= interval && this.enemies.length < max) {
      this.spawnAcc = 0;
      let amount = 10 + Math.floor(minute / 1.8);
      if (minute >= 5) amount += 2;
      if (minute >= 10) amount += 2;
      if (minute >= 15) amount += 2;
      if (this.hordeWaveTimer > 0) amount += 12;
      if (this.challenge?.active) amount += 3;
      amount = Math.max(10, Math.min(34, amount));
  ...
  spawnGroup(amount) {
    ...
    const minute = this.time / 60;
    let archersInGroup = 0;
    for (let i = 0; i < amount && this.enemies.length < this.enemyCap(); i++) {
      let type = this.chooseEnemyForGroup(minute, i, amount);
      if (type === 'boneArcher') {
        if (archersInGroup >= 1) type = minute < 10 ? 'skeleton' : 'orc';
        else archersInGroup++;
      }
  ...
  chooseEnemyForGroup(minute, index, amount) {
    // Hordas maiores vêm principalmente de melee/básicos; Arqueiro Ósseo tem peso próprio baixo.
    const r = Math.random();
    if (this.hordeWaveTimer > 0 && this.hordeWaveType === 'boneTide') return r < .86 ? 'skeleton' : r < .93 ? 'goblin' : r < .975 ? 'summoner' : 'boneArcher';
    ...
    if (minute < 2) return r < .50 ? 'slime' : r < .76 ? 'goblin' : r < .92 ? 'bat' : 'skeleton';
    if (minute < 5) return r < .38 ? 'slime' : r < .61 ? 'goblin' : r < .81 ? 'skeleton' : r < .98 ? 'bat' : 'boneArcher';
    if (minute < 10) return r < .38 ? 'skeleton' : r < .56 ? 'goblin' : r < .72 ? 'bat' : r < .75 ? 'bat' : r < .785 ? 'boneArcher' : r < .91 ? 'orc' : 'bomber';
    if (minute < 15) return r < .34 ? 'skeleton' : r < .50 ? 'orc' : r < .64 ? 'darkMage' : r < .675 ? 'bat' : r < .705 ? 'boneArcher' : r < .82 ? 'bomber' : r < .92 ? 'summoner' : 'stoneGolem';
    return r < .22 ? 'orc' : r < .40 ? 'skeleton' : r < .52 ? 'darkMage' : r < .555 ? 'specter' : r < .58 ? 'boneArcher' : r < .69 ? 'bomber' : r < .79 ? 'summoner' : r < .89 ? 'demon' : r < .96 ? 'specter' : 'stoneGolem';
  }

  boneArcherCap() {
    const minute = this.time / 60;
    if (minute < 4) return 2;
    if (minute < 10) return 4;
    if (minute < 18) return 6;
    return 7;
  }

  livingBoneArchers() {
    let count = 0;
    for (const e of this.enemies) if (!e.dead && e.type === 'boneArcher') count++;
    return count;
  }
  ...
  chooseEnemy() {
    ...
    if (m < 5) return r < .27 ? 'slime' : r < .52 ? 'goblin' : r < .78 ? 'skeleton' : r < .96 ? 'bat' : 'boneArcher';
    if (m < 9) return r < .21 ? 'goblin' : r < .46 ? 'skeleton' : r < .65 ? 'orc' : r < .80 ? 'darkMage' : r < .96 ? 'bomber' : 'boneArcher';
    if (m < 14) return r < .22 ? 'orc' : r < .40 ? 'darkMage' : r < .55 ? 'summoner' : r < .70 ? 'bomber' : r < .84 ? 'skeleton' : 'stoneGolem';
    return r < .21 ? 'stoneGolem' : r < .40 ? 'summoner' : r < .59 ? 'darkMage' : r < .76 ? 'bomber' : r < .95 ? 'orc' : 'boneArcher';
  }
  ...
  eliteChance() {
    ...
    return Math.min(.21, (.006 + m * .0028) * this.difficulty.elite * mapBonus * curseBonus * (this.flags.hungryCrown ? 1.35 : 1));
  }
  ...
  spawnEnemy(type, forceElite = false, around = null) {
    if (type === 'boneArcher' && this.livingBoneArchers() >= this.boneArcherCap()) {
      type = this.time < 600 ? (Math.random() < .55 ? 'skeleton' : 'goblin') : (Math.random() < .55 ? 'skeleton' : 'orc');
    }
  ...
  bossPool() {
    return [
      'graveTyrant','arcaneBehemoth','plagueMother','frostColossus',
      'infernalWyrm','stormHerald','titanRoots','voidReaper',
      'ogreBoss','lichBoss','wardenBoss','beastBoss'
    ];
  }

  refillBossDeck() {
    const pool = this.bossPool().slice();
    this.shuffle(pool);
    this.bossDeck = pool;
    this.bossDeckIndex = 0;
  }
  ...
  nextBossType() {
    if (!this.bossDeck.length || this.bossDeckIndex >= this.bossDeck.length) this.refillBossDeck();
    return this.bossDeck[this.bossDeckIndex++];
  }

  activeBoss() {
    return this.enemies.find(e => e.boss && !e.dead) || null;
  }

  updateBossSchedule() {
    // Um boss é agendado a cada 2 minutos. Se o anterior ainda estiver vivo, entra na fila.
    while (this.time >= this.nextBossTime) {
      const isTitanWindow = Math.abs(this.nextBossTime - GAME_CONFIG.finalBossTime) < .5;
      const type = isTitanWindow && !this.finalSpawned ? 'finalBoss' : this.nextBossType();
      this.bossQueue.push(type);
      if (type === 'finalBoss') this.finalSpawned = true;
      this.nextBossTime += 120;
    }

    if (!this.activeBoss() && this.bossQueue.length) {
      this.spawnBoss(this.bossQueue.shift());
    }
  }
  ...
  spawnBoss(type) {
    ...
    const free = this.world?.findNearestFree(x, y, (cfg.hitboxRadius || cfg.size || 30) + 8) || { x, y };
    const e = new Enemy(type, cfg, free.x, free.y, this.enemyScaling(type));

    // Bosses tardios ganham complexidade/pressão sem virar apenas sacos de HP.
    const cycle = Math.max(1, Math.floor(this.time / 120));
    e.maxHp *= (type === 'finalBoss' ? 1.3 : 1) * (1 + Math.min(.55, Math.max(0, cycle - 1) * .045)) * (this.flags.cursedHourglass ? 1.16 : 1);
    e.hp = e.maxHp;
    e.damage *= 1 + Math.min(.35, Math.max(0, cycle - 1) * .025);
    e.bossCycle = cycle;
    e.scheduledBoss = true;
    this.enemies.push(e);
    this.save.discoverBoss?.(type);
    const tracked=this.trackedFusionId ? ARCANE_FUSIONS.find(r=>r.id===this.trackedFusionId) : null;
    if (tracked?.item && FUSION_ITEMS[tracked.item]?.source===type) this.ui.flashMessage(`⚠ ITEM DE FUSÃO DETECTADO • ${e.name} → ${FUSION_ITEMS[tracked.item].name}`);
    this.bossSequence++;
    this.ui.flashMessage(`⚠ A PRESENÇA PODEROSA FOI DETECTADA • ${e.name}`);
    return e;
  }
  ...
  ensureSummons(id, count, damage, electric = false, variant = null, options = {}) {
    ...
    while (existing.length < count && this.summons.length < 36) {
      ...
      const s = {
        ...
        orbitalLight: !!options.orbitalLight,
        projectileCount: options.projectileCount || 1,
        orbitRadius: options.orbitRadius || 64,
        doubleRing: !!options.doubleRing,
        projectileExplode: options.projectileExplode || 0,
    ...
    for (const s of existing) {
      ...
      s.orbitalLight = s.orbitalLight || !!options.orbitalLight;
      s.projectileCount = Math.max(s.projectileCount || 1, options.projectileCount || 1);
      s.orbitRadius = Math.max(s.orbitRadius || 64, options.orbitRadius || 64);
      s.doubleRing = s.doubleRing || !!options.doubleRing;
      s.projectileExplode = Math.max(s.projectileExplode || 0, options.projectileExplode || 0);
    }
  }
  ...
  updateSummons(dt) {
    ...
    for (const s of this.summons) {
      ...
      if (s.orbitalLight) {
        s.orbitAngle = (s.orbitAngle || 0) + dt * (1.65 + Math.min(.8, (s.projectileCount || 1) * .08));
        const radius = s.orbitRadius || 64;
        s.x = p.x + Math.cos(s.orbitAngle) * radius;
        s.y = p.y + Math.sin(s.orbitAngle) * radius;
        if (!s.target || s.target.dead || Math.hypot(s.target.x - s.x, s.target.y - s.y) > 720) s.target = this.nearestEnemy(s.x, s.y, 720);
        if (s.target && s.cd <= 0) {
          const dx=s.target.x-s.x, dy=s.target.y-s.y, base=Math.atan2(dy,dx);
          const shots=Math.max(1,Math.min(9,s.projectileCount||1));
          for(let i=0;i<shots;i++){
            const spread=(i-(shots-1)/2)*Math.min(.15,.46/Math.max(1,shots-1));
            this.spawnProjectile({id:s.id,x:s.x,y:s.y,vx:Math.cos(base+spread)*455,vy:Math.sin(base+spread)*455,r:6,damage:s.damage,element:s.variant==='fire'?'fire':s.electric?'electric':'arcane',pierce:s.doubleRing?1:0,explode:s.projectileExplode||0,chainOnHit:s.electric?(s.chainCount||1):0,chainRange:s.chainRange||165,life:2.4});
          }
          s.cd = s.electric ? .50 : s.doubleRing ? .54 : .68;
        }
        keep.push(s);
        continue;
      }
  ...
  damageEnemy(enemy, damage, canCrit = true, element = 'physical', number = true, sourceId = element) {
    ...
    enemy.hp -= damage;
    if (this.flags.emberCore && element !== 'fire' && Math.random() < .10) {
      this.recordItemUse('emberCore','burnProcs',1);
      enemy.status.burn = Math.max(enemy.status.burn || 0, 1.8);
      enemy.status.burnDps = Math.max(enemy.status.burnDps || 0, Math.max(4, damage * .06));
    }
    enemy.hitFlash = .075;
    this.player.damageDone += damage;
    this.recordDamage(sourceId, damage);
    ...
    if (crit && this.flags.voidEye && Math.random() < .075) {
      this.recordRelicUse('voidEye','activations',1);
  ...
  killEnemy(enemy, sourceId = 'unknown') {
    ...
    this.player.kills++;
    this.sourceKills ||= {};
    this.sourceKills[sourceId] = (this.sourceKills[sourceId] || 0) + 1;
    if (this.flags.deathBell) { this.flags.deathBellKills=(this.flags.deathBellKills||0)+1; if(this.flags.deathBellKills>=45){this.flags.deathBellKills=0;this.recordItemUse('deathBell','activations',1);this.areaHit(this.player.x,this.player.y,220,62*this.player.damage,'arcane',0,false,'item:deathBell');this.addAbilityEffect({type:'arcaneBurst',x:this.player.x,y:this.player.y,radius:220,life:.45,maxLife:.45});} }
    ...
    if (enemy.elite && Math.random() < .11) this.spawnChest(enemy.x, enemy.y, this.rollChestTier('elite'));
    // Como baús são raros, elites ainda podem conceder um item diretamente sem
    // transformar cada grupo em uma chuva de baús.
    if (enemy.elite && Math.random() < .075) this.grantRandomRunItem('rare');
    if (enemy.boss) {
      this.spawnChest(enemy.x, enemy.y, this.rollChestTier(enemy.type === 'finalBoss' ? 'final' : 'boss'));
      this.tryBossFusionItemDrop(enemy);
      this.grantRandomRunItem(enemy.type === 'finalBoss' ? 'legendary' : 'epic');
    }

    if (this.flags.glacialCrown && wasFrozen) {
      this.recordRelicUse('glacialCrown','activations',1);
    ...
    if (this.flags.bookDead && Math.random() < .12) { this.recordRelicUse('bookDead','summons',1); this.spawnRevenant(enemy.x, enemy.y, 30 * this.player.damage); }
  ...
  collectPickup(p) {
    ...
    else if (p.type === 'fusionItem') this.addFusionItem(p.itemId);
    else if (p.type === 'runItem') this.addRunItem(p.itemId);
  }
  ...
  healPlayer(amount) {
    ...
    if (healed > 0 && this.stats) this.stats.healingReceived = (this.stats.healingReceived || 0) + healed;
    return healed;
  }

  recordDamage(sourceId = 'unknown', amount = 0) {
    if (!(amount > 0)) return;
    this.weaponDamage ||= {};
    this.damageHistory ||= {};
    const id = sourceId || 'unknown';
    this.weaponDamage[id] = (this.weaponDamage[id] || 0) + amount;
    const list = this.damageHistory[id] || (this.damageHistory[id] = []);
    list.push({ t: this.time || 0, damage: amount });
    const cutoff = (this.time || 0) - 8;
    while (list.length && list[0].t < cutoff) list.shift();
    if (list.length > 220) list.splice(0, list.length - 220);
  }

  recordItemUse(id, metric = 'activations', amount = 1) {
    const stats = this.itemStats[id] || (this.itemStats[id] = {});
    stats[metric] = (stats[metric] || 0) + amount;
  }

  recordRelicUse(id, metric = 'activations', amount = 1) {
    const stats = this.relicStats[id] || (this.relicStats[id] = {});
    stats[metric] = (stats[metric] || 0) + amount;
  }
  ...
  spawnFusionItem(x, y, itemId) {
    const item = FUSION_ITEMS[itemId];
    if (!item) return;
    this.pickups.push({ type:'fusionItem', itemId, x, y, value:1, life:95, rarity:item.rarity });
    this.ui.flashMessage(`ITEM DE FUSÃO DERRUBADO • ${item.icon} ${item.name}`);
  }

  tryBossFusionItemDrop(enemy) {
    const candidates = enemy.fusionItems?.length ? enemy.fusionItems : enemy.fusionItem ? [enemy.fusionItem] : [];
    for (const itemId of candidates) {
      if (!FUSION_ITEMS[itemId]) continue;
      const pity = this.fusionItemPity[itemId] || 0;
      const guaranteed = pity >= 1; // no máximo duas derrotas da fonte relacionada
      const chance = FUSION_ITEMS[itemId].source === enemy.type ? 1 : .42;
      if (guaranteed || Math.random() < chance) {
        this.fusionItemPity[itemId] = 0;
        this.spawnFusionItem(enemy.x + 22 + Math.random()*18, enemy.y - 8 + Math.random()*16, itemId);
      } else {
        this.fusionItemPity[itemId] = pity + 1;
      }
    }
  }

  addFusionItem(itemId) {
    if (!FUSION_ITEMS[itemId]) return;
    this.player.addFusionItem(itemId, 1);
    this.save.discoverFusionItem?.(itemId);
    this.ui.refreshFusionItems?.(this.player.fusionItems);
    this.ui.flashMessage(`FUSION ITEM OBTIDO • ${FUSION_ITEMS[itemId].icon} ${FUSION_ITEMS[itemId].name}`);
    this.checkTransformations();
  }

  addRunItem(itemId) {
    if (!RUN_ITEMS[itemId] || this.player.items.includes(itemId)) return false;
    applyRunItem(this, itemId);
    this.ui.refreshItems?.(this.player.items);
    this.ui.flashMessage(`ITEM OBTIDO • ${RUN_ITEMS[itemId].icon} ${RUN_ITEMS[itemId].name}`);
    return true;
  }

  grantRandomRunItem(tier='rare') {
    const rolled=rollRunItem(this.player.items,tier);
    if (!rolled) return false;
    return this.addRunItem(rolled[0]);
  }

  updateRunItems(dt) {
    this.itemAuraTimer=(this.itemAuraTimer||0)-dt;
    if (this.itemAuraTimer>0) return;
    this.itemAuraTimer=.65;
    if (this.flags.frostRing) for (const e of this.closestEnemies(this.player.x,this.player.y,150,36)) if(!e.boss) e.status.freeze=Math.max(e.status.freeze||0,.18);
  }
  ...
  openChest(tier = 'common') {
    ...
    // Baús são mais raros, então priorizam recompensas realmente úteis.
    if (tierRank >= 2 && Math.random() < .18) {
      const missing = Object.entries(FUSION_ITEMS).filter(([id]) => !(this.player.fusionItems[id] > 0));
      if (missing.length) {
        const [id,it] = missing[(Math.random()*missing.length)|0];
        reward = {type:'fusionItem', id, icon:it.icon, name:it.name, desc:`Fusion Item • ${it.desc}`, rarity:it.rarity};
      }
    }
    if (!reward && Math.random() < .52) {
      const item = rollRunItem(this.player.items, tier);
      if (item) reward = {type:'runItem', id:item[0], icon:item[1].icon, name:item[1].name, desc:item[1].desc, rarity:item[1].rarity};
    }
    if (!reward && Math.random() < .62) {
      const relic = rollRelic(this.player.relics, tier);
      if (relic) reward = { type:'relic', id:relic[0], icon:relic[1].icon, name:relic[1].name, desc:relic[1].desc, rarity:relic[1].rarity };
    }
    if (!reward) {
      const choices=this.getChoices(1), c=choices[0];
      if (c && Math.random()<.58) reward={...c,type:'upgrade'};
      else reward={type:'coins',icon:'◈',name:'Tesouro Arcano',desc:`+${35+tierRank*18} moedas`,amount:35+tierRank*18,rarity:tier};
    }

    this.ui.showChest(reward, () => { this.applyChestReward(reward); this.paused=false; });
  }

  applyChestReward(reward) {
    if (!reward) return;
    if (reward.type === 'relic') this.addRelic(reward.id);
    else if (reward.type === 'runItem') this.addRunItem(reward.id);
    else if (reward.type === 'fusionItem') this.addFusionItem(reward.id);
  ...
  showLevelSession() {
    ...
    this.ui.showLevelUp(session.choices, {
      ...
      onBanish: choice => {
        if (this.player.banishes <= 0 || choice.signature) return;
  ...
  getChoices(count = 4) {
    ...
    for (const [id, w] of Object.entries(WEAPONS)) {
      ...
      if (w.signature && w.signatureClass !== p.id) continue;
      ...
      if (w.fusion) {
        if (level > 0 && level < w.max) {
          options.push({
            ...
            rarity: 'arcane', fusion: true, weight: 1
          });
        }
        continue;
      }
      ...
      if (level > 0 && level < w.max) {
        ...
        options.push({
          ...
          rarity: this.rollRarity(), signature: !!w.signature, weight: w.signature ? 4 : abilityBias(p.id, w)
        });
    ...
    for (const [id, passive] of Object.entries(PASSIVES)) {
      ...
      if (level < passive.max && (level > 0 || p.passiveOrder.length < 6)) {
        options.push({
          ...
          rarity: this.rollRarity(), weight: 1
        });
      }
    }

    return this.weightedChoices(options, count);
  }

  weightedChoices(options, count) {
    const pool = options.slice(), out=[];
    while (pool.length && out.length<count) {
      let total=pool.reduce((a,c)=>a+(c.weight||1),0), r=Math.random()*total, idx=0;
  ...
  applyChoice(choice) {
    ...
    if (choice.kind === 'weapon') {
      if (!p.weapons[choice.id]) { p.addWeapon(choice.id, 1); this.save.discoverAbility?.(choice.id); }
  ...
  applyTransformation(recipe, automatic = false) {
    if (!recipe) return;
    if (recipe.fusion && recipe.item && !this.player.hasFusionItem(recipe.item)) return;
    const oldNames = (recipe.replace || []).map(id => WEAPONS[id]?.name || id).join(' + ');
    if (recipe.fusion && recipe.item) this.player.consumeFusionItem(recipe.item);
    const resultId = replaceAbility(this.player, recipe);
    this.save.discoverAbility?.(resultId);
    if (recipe.fusion) {
  ...
  updateRelicTimers(dt) {
    if (this.flags.arcaneMagnet) {
      ...
      if (this.magnetTimer <= 0) {
        this.magnetTimer = 45;
        this.magnetAllXp();
        this.recordRelicUse('arcaneMagnet','activations',1);
  ...
  onAbilityCast(id) {
    ...
    if (this.flags.stormBattery) {
      this.flags.stormBatteryCount=(this.flags.stormBatteryCount||0)+1;
      if (this.flags.stormBatteryCount>=8) { this.flags.stormBatteryCount=0; this.recordItemUse('stormBattery','activations',1); const targets=this.closestEnemies(this.player.x,this.player.y,620,5); let prev=this.player; for(const e of targets){this.damageEnemy(e,34*this.player.damage,false,'electric',true,'item:stormBattery');this.lightningArcs.push({x1:prev.x,y1:prev.y,x2:e.x,y2:e.y,life:.12,color:'#ffe45b'});prev=e;} }
    }
    if (this.flags.gravityStone && Math.random()<.08) { this.recordItemUse('gravityStone','activations',1); for(const e of this.closestEnemies(this.player.x,this.player.y,190,28)){if(e.boss)continue;const dx=this.player.x-e.x,dy=this.player.y-e.y,d=Math.hypot(dx,dy)||1;e.x+=dx/d*16;e.y+=dy/d*16;} }
  }
  ...
  drawPickup(c, p) {
    const colors = { xp: '#56d7ff', coin: '#ffd45c', heal: '#75ef91', magnet: '#d773ff', chest: '#d6ae52', fusionItem: '#ff82e9' };
    const size = p.type === 'chest' ? 12 : 6;
    this.drawCircleWorld(c, p.x, p.y, size, colors[p.type] || '#fff');
    if ((p.type === 'chest' || p.type === 'fusionItem') && this.camera.visible(p.x, p.y, 20)) {
      const s = this.camera.worldToScreen(p.x, p.y);
      c.font = p.type === 'fusionItem' ? '17px system-ui' : '14px system-ui';
      c.textAlign = 'center';
      c.fillText(p.type === 'fusionItem' ? (FUSION_ITEMS[p.itemId]?.icon || '✦') : '🧰', s.x, s.y + 5);
    }
  }
