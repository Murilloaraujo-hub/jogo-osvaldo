import { WEAPONS, getWeaponStats } from './weaponData.js?v=2.9.0';
...
export class WeaponSystem {
  ...
  fire(id, w, level, fromChaos = false) {
    ...
    if (!fromChaos && g.flags.echoCrystal && Math.random() < .09) {
      g.recordItemUse?.('echoCrystal','activations',1);
    ...
    if (!fromChaos && g.flags.infinityOrb) {
      ...
      if(this.infinityCounter>=14){this.infinityCounter=0;g.recordItemUse?.('infinityOrb','activations',1);const copyStats={...w,damage:(w.damage||0)*.62};g.delayedEffects.push({t:.2,fn:()=>this.fire(id,copyStats,level,true)});}
    }
  ...
  fireProjectile(id, w, level, power, countBonus) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (g.flags.mirrorRune && Math.random() < .14) {
        if (g.player.items?.includes('mirrorRuneItem')) g.recordItemUse?.('mirrorRuneItem','extraProjectiles',1);
        if (g.player.relics?.includes('mirrorRune')) g.recordRelicUse?.('mirrorRune','extraProjectiles',1);
