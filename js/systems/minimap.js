...
export class MinimapSystem{
  ...
  collectObjectives(){
    ...
    for(const e of g.enemies||[]){if(e.dead)continue;if(e.boss){const isMinor=e.miniboss&&!e.scheduledBoss;arr.push({type:isMinor?'miniboss':'boss',x:e.x,y:e.y,label:isMinor?'MINIBOSS':(e.name||'BOSS').toUpperCase(),priority:isMinor?8:10});}else if(e.elite)arr.push({type:'elite',x:e.x,y:e.y,label:'ELITE',priority:1});}
    return arr;
  }
