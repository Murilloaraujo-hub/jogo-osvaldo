import { CHARACTERS, MAPS, META_UPGRADES, DIFFICULTIES, ACHIEVEMENTS } from '../config.js?v=2.6.0';
import { WEAPONS } from '../weapons/weaponData.js?v=2.6.0';
import { EVOLUTIONS, ARCANE_FUSIONS } from '../evolutions.js?v=2.6.0';
import { RELICS } from '../relics.js?v=2.6.0';
import { CLASS_PASSIVES } from '../systems/classPassives.js?v=2.6.0';
import { drawClassPreview } from '../visuals/playerVisuals.js?v=2.6.0';
  ...
  renderGrimoire() {
    ...
    for (const e of rows) {
      ...
      const recipeText = e.arcane && e.abilities
        ? `${WEAPONS[e.abilities[0]]?.name || '???'} + ${WEAPONS[e.abilities[1]]?.name || '???'} → ${WEAPONS[e.result]?.name || e.name}`
        : e.desc;
      d.innerHTML = e.known
